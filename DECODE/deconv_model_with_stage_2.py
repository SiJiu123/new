...
class MBdeconv(nn.Module):
    def __init__(self, num_MB, feat_map_w, feat_map_h, num_cell_type, epoches, Alpha, Beta, train_data, test_data, domain_ep=25, freeze_encoder=True, denoiser_mode='normal'):
        super().__init__()
        self.num_MB = num_MB
        self.feat_map_w = feat_map_w
        self.feat_map_h = feat_map_h
    ...
    def forward(self, mix):
        ...
        if self.denoiser_mode == 'normal':
            mask_res = self.masknet(enc_mix)
            noise = mask_res[0]
            extract_cell = mask_res[1]
        elif self.denoiser_mode == 'swap':
            mask_res = self.masknet(enc_mix)
            noise = mask_res[1]
            extract_cell = mask_res[0]
        elif self.denoiser_mode == 'no_mask':
            noise = torch.zeros_like(enc_mix)
            extract_cell = enc_mix
        else:
            raise ValueError(f"Unsupported denoiser_mode: {self.denoiser_mode}")
