...
from demo2 import demo2 as DemoModel
...
OUT_PRED = "demo2_preds.csv"
OUT_METRICS = "demo2_metrics_summary.csv"
OUT_TYPEWISE = "demo2_typewise_ccc.csv"
OUT_PROTO_DIR = "demo2_prototype_vis"
...
def load_test_bulk_from_302c(bulk_file, gene_order, cell_types):
    ...
    # Placeholder labels: required by demo2.py dataloader shape, not used for training.
...
def main():
    if not torch.cuda.is_available():
        raise RuntimeError("demo2.py uses cuda directly. Please run on a CUDA-enabled machine.")
    ...
    proto_vis = model_da.visualize_final_prototypes(source_data, out_dir=OUT_PROTO_DIR)
    ...
    print(f"Saved: {proto_vis['scatter_path']}")
    print(f"Saved: {proto_vis['score_hist_path']}")
    print(f"Saved: {proto_vis['detail_csv']}")
    print(f"Saved: {proto_vis['topk_csv']}")
    print(
        "Prototype summary | "
        f"topk={proto_vis['k_size']}, "
        f"topk score mean={proto_vis['topk_score_mean']:.6f}, "
        f"all score mean={proto_vis['all_score_mean']:.6f}"
    )
