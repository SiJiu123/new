...
class PrototypeBank(nn.Module):
    def __init__(self, k_size, feature_dim, label_dim):
        super().__init__()
        # Use dynamic tensors instead of fixed register_buffer
        self.feature = torch.zeros(1, feature_dim).cuda()
        self.label = torch.zeros(1, label_dim).cuda()
        self.k_size = 1

    def update(self, new_features, new_labels):
        self.feature = new_features.clone()
        self.label = new_labels.clone()
        self.k_size = new_features.shape[0]
...
def RefreshPrototype(model_list, dataloder, bank, score_threshold=0.98, max_per_class=10, mixed_margin=0.25):
        ...
        num_classes = full_y.shape[1]
        
        # Manifold Stratification
        max_vals, _ = torch.max(full_y, dim=1)
        min_vals, _ = torch.min(full_y, dim=1)
        ranges = max_vals - min_vals
        
        dominant_classes = torch.argmax(full_y, dim=1)
        
        # Mixed camp ID = num_classes
        camp_ids = torch.where(ranges < mixed_margin, torch.tensor(num_classes).cuda(), dominant_classes)
        
        selected_indices = []
        
        # Quality & Diversity Gating
        for camp_id in range(num_classes + 1):
            camp_mask = (camp_ids == camp_id) & (full_scores > score_threshold)
            camp_indices = torch.nonzero(camp_mask).squeeze(-1)
            
            if len(camp_indices) > 0:
                camp_scores = full_scores[camp_indices]
                K = min(max_per_class, len(camp_indices))
                _, top_k_idx = torch.topk(camp_scores, k=K, largest=True)
                selected_indices.append(camp_indices[top_k_idx])
        
        # Fallback Mechanism
        if len(selected_indices) > 0:
            final_indices = torch.cat(selected_indices, dim=0)
        else:
            _, top_1_idx = torch.topk(full_scores, k=1, largest=True)
            final_indices = top_1_idx
            
        new_features = full_x[final_indices].cuda()
        new_labels = full_y[final_indices].cuda()
        ...
        if old_labels.shape == new_labels.shape:
            ...
            update_magnitude = 0.0 # Magnitude not strictly defined when size changes

        avg_bank_score = torch.mean(full_scores[final_indices]).item()

        return update_magnitude, avg_bank_score, bank.k_size
...
class demo3(object):
    ...
    def demo3_model(self, celltype_num):
    ...
    def train(self, source_data, target_data, valid_data, patience):
        ...
        self.model_da = self.demo3_model(self.celltype_num).cuda()
        ...
        for epoch in range(self.num_epochs):
            ...
            if epoch > 5:
                up_mag, bank_score, bank_size = RefreshPrototype(self.model_da, self.train_source_loader, self.bank)
                print(f" 🔍 {Fore.YELLOW}[Bank Monitor]{Style.RESET_ALL} "
                    f"Size: {Fore.CYAN}{bank_size}{Style.RESET_ALL} | "
    ...
    def visualize_final_prototypes(self, source_data, out_dir="demo3_prototype_vis"):
        ...
        plt.hist(scores[is_prototype], bins=min(20, max(1, k_size)), alpha=0.8, label='prototype scores')
